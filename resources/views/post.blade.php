...
@section('content')
    ...
    @if(count($post->images) > 0)
        <div class="bg-contain bg-center bg-no-repeat min-h-full h-96" style="background-image: url('{{ asset("storage/" . $post->images->random()->path) }}')"></div>
    @else
    ...
    @endif
    <div class="mx-auto max-w-7xl px-6 lg:px-8 mt-8 mb-10 sm:mb-16">
        {!! $post->text !!}
    </div>
@endsection
